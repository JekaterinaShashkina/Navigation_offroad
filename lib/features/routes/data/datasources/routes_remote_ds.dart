...
CollectionReference<Map<String, dynamic>> get _col => _db.collection('routes');
  ...
  Future<List<RouteModel>> fetchVisibleForUser(String userId) async {
    try {
      final publicSnap =
          await _col.where('isPublic', isEqualTo: true).get();
      final mySnap =
          await _col.where('ownerId', isEqualTo: userId).get();
...
try {
final snap = await _col.where('ownerId', isEqualTo: ownerId).orderBy('createdAt', descending: true).get();
...
try {
await _col.doc(id).update({'isPublic': isPublic});
} on FirebaseException catch (e) {
throw ServerFailure(e.message ?? 'Firestore error');
}
}
